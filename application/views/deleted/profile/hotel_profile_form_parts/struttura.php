...
<div style="margin-bottom:0px;">
	<div id="save_struttura_message" class="success_message"></div>
                                        ...
                                        <label class="padding_top_7"><?php echo lang('type_of_hotel');?></label>
											<select name="root_category" id="root_category" class="styled">
												<?php foreach($rootcategories as $rootcategory_key=>$rootcategory) { ?>
                                                	<option value="<?php echo $rootcategory->category_id;?>" <?php if(isset($root_category) && $root_category==$rootcategory->category_id){ ?> selected="selected" <?php } ?><?php if(isset($profile_details) && $profile_details->category==$rootcategory->category_id){ ?> selected="selected" <?php } ?> ><?php echo $rootcategory->category_name;?> </option>
                                                <?php } ?>
											</select>
										<div class="clearfix"></div>
                                        
                                        <div id="load_subcategory">
											<?php echo $this->template->block('SubCategory','profile/hotel_profile_form_parts/subcategories.php');?>
                                        </div>
                                        
                                        
                                        <input type="hidden" name="hotel_rating" value="1">
                                        ...
                                        <div class="clearfix-big"></div>
					                    <label>&nbsp;</label>
										 <input type="checkbox" name="accept_privacy_conditions" id="accept_privacy_conditions" value="1" <?php if((isset($accept_privacy) && $accept_privacy=="1") || (isset($profile_details) && $profile_details->accept_privacy=="1") || true){ ?> checked="checked" <?php } ?> style="width:30px; margin-left:-5px;"><?php echo lang('i_agree_text');?> <a href="#privacy_policy" rel="leanModal"><?php echo lang('privacy');?> </a>
										<div class="clearfix"></div>
										
										<div class="clearfix"></div>
					                    <label>&nbsp;</label>
										 <input type="checkbox" name="send_newsletter" id="send_newsletter" value="1" <?php if ((isset($send_newsletter) && $send_newsletter=="1")|| (isset($profile_details) && $profile_details->send_newsletter=="1") || true) { ?> checked="checked" <?php } ?> style="width:30px; margin-left:-5px;"> Notify me by e-mail about travelly's offers and news
										<div class="clearfix"></div>
                                        </div>
