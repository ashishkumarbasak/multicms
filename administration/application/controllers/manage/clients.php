...
class Clients extends CI_Controller {
	...
	public function __construct(){
			...
			$this->load->model('manage/document_model');
	...
	function index(){
		...
		$total_users = $this->usermodel->get_total_users($user_type='2');
		
		$config['base_url'] = base_url()."manage/clients/page";
		...
		$all_users = $this->usermodel->get_all_users($this->per_page,$offset,$user_type='2');
		$this->template->assign('userlist',$all_users);
		
		$this->template->assign('page','manage/clients/index.tpl');	
	...
	function create(){
		...
		$this->template->assign('account_type','2');
		$this->template->assign('page','manage/clients/create.tpl');	
		$this->template->display('layouts/layout.tpl');
	}
		...
		if($user_id!=NULL){
			...
			if($this->input->post('update_my_account')){
				...
				if($submitok){ 
					...
					//$this->usermodel->save_user_documents($this->myvalidation->data,$user_id);
			...
			$user_files = $this->document_model->get_client_documents($user_id);
			$this->template->assign('user_files',$user_files);
			
			$this->template->assign('show_upload_file_button','yes');
			
			$this->template->assign('page','manage/clients/edit.tpl');	
			$this->template->display('layouts/layout.tpl');
		}
		else{
			redirect('manage/members','refresh');
	...
	function delete_document($file_id=NULL,$user_id=NULL){
		if($file_id!=NULL && $user_id!=NULL){
			$this->document_model->delete_client_file($file_id,$user_id);
			redirect('/manage/clients/edit/'.$user_id , 'refresh');
		}
	}
}
?>