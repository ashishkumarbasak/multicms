...
class Documents extends CI_Controller {
	...
	function index(){
		...
		$total_documents = $this->document_model->get_total_documents();
		
		$config['base_url'] = base_url()."manage/documents/page";
		...
		$all_documents = $this->document_model->get_all_documents($this->per_page,$offset);
		$this->template->assign('documentlist',$all_documents);
		
		$this->template->assign('page','manage/documents/index.tpl');	
		...
		if($document_id!=NULL){
			...
			if($this->input->post('update_document')){
				...
				if($submitok){
					...
					$this->document_model->update_document($this->myvalidation->data,$document_id);
					//$toemail=$this->myvalidation->data['email_address'];				
					//$replace['verify_link']=base_url().'users/verifyemail/'.md5($user_id).'/'.md5($this->myvalidation->data['email_address']);
					//$this->myemaillibrary->set_email_category('user_signup');
					//$this->myemaillibrary->send_email($toemail,$replace);
			...
			$this->template->assign('page','manage/documents/edit.tpl');	
			$this->template->display('layouts/layout.tpl');
		}
		else{
			redirect('manage/documents','refresh');
