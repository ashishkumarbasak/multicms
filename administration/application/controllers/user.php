...
class User extends CI_Controller {
	...
	function login()
	{
		if($this->userauthentication->is_loggedin()){
		...
		$display_error=$this->phpsession->get('admin_display_error');
		$this->template->assign('admin_display_error',$display_error);
		$this->template->assign('admin_error_message',$this->phpsession->get('admin_error_message'));
		$this->phpsession->clear('admin_display_error');
		$this->phpsession->clear('admin_error_message');
		//$this->template->assign('reference',$this->uri->segment(1)); //will use it later for reference login
		$login_fail_counter = $this->phpsession->get('admin_login_fail_counter');
