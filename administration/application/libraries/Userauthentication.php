...
class Userauthentication
{
    ...
    function Userauthentication()
    {
		...
		$this->obj->load->model(array('user_model','user_profile_model','manage/language_model'));
		$this->is_loggedin();
		$all_languages = $this->obj->language_model->get_all_languages(NULL, NULL);
		$this->obj->template->assign('langlist',$all_languages);
    }
		...
		if($recaptcha_validate=="yes")
			{
				...
				if (!$resp->is_valid) 
				{
  					if($this->obj->phpsession->get('admin_login_fail_counter'))
					$this->login_fail_counter = $this->obj->phpsession->get('admin_login_fail_counter');
					$this->login_fail_counter++;
					$this->obj->phpsession->save('admin_login_fail_counter',$this->login_fail_counter);
				
					$this->obj->phpsession->save('admin_display_error','yes');
					$this->obj->phpsession->save('admin_error_message',$this->obj->lang->line('admin_login_fail_for_captcha'));
		...
		if($auth_result)
			{
				...
				$this->obj->phpsession->save('admin_loggedin_username', $loggedin_username);
				$this->obj->phpsession->save('admin_no_of_dashboard_visit', $this->no_of_dashboard_visit);
				$this->obj->phpsession->save('admin_user_authentication_counter', $this->user_authentication_counter);
			    ...
			    if ($autoLogin)
					...
					{
						delete_cookie("ecellenzia_admin_usercookie");
						delete_cookie("ecellenzia_admin_pcookie");
						$remember_me='0';
					}
		...
		else
			{
				...
				if($redirect)
				{
					...
					//exit(0);
					$this->obj->phpsession->save('admin_display_error','yes');
					$this->obj->phpsession->save('admin_error_message',$this->obj->lang->line('login_fail_error'));
					redirect('user/login','refresh');
				}
				else
					return false;
    ...
    {
		$this->obj->phpsession->get('admin_loggedin_username');
		//exit(0);
		if($this->obj->phpsession->get('admin_loggedin_username'))
			{
				...
				if($profile_details!=NULL)
						$this->obj->template->assign('admin_profile_details',$profile_details[0]);

				$this->obj->template->assign('admin_is_loggedin','true');
				$this->obj->template->assign('admin_loggedin_username',$this->username);
				$this->obj->template->assign('admin_loggedin_usertype',$this->usertype);
				$this->obj->template->assign('admin_display_name',$this->obj->user_model->get_loggedin_display_name($this->username));
		...
		else
			{
				...
				$username_from_cookie = get_cookie('ecellenzia_admin_usercookie');
				$this->obj->template->assign('admin_username_from_cookie',$username_from_cookie);
				$password_from_cookie = get_cookie('ecellenzia_admin_pcookie');
				...
				{
					$this->obj->template->assign('admin_is_loggedin','false');
					return false;
				}
			}
    }
	...
	function is_exist_hotelusername($username)
	{
		$is_exist=$this->obj->user_model->is_exist_hotelusername($username);
		if($is_exist)
		return true;
		else
		return false;
	...
	function is_exist_hotelemail($email_address)
	{
		$is_exist=$this->obj->user_model->is_exist_hotelemail($email_address);
		if($is_exist)
			return true;
		else
			return false;
    ...
    {
    	$this->username=$this->obj->phpsession->get('admin_loggedin_username');
    }
	...
	function check_sessionexpire()
	{
		...
		{
			$this->obj->phpsession->save('admin_display_error','yes');
			$this->obj->phpsession->save('admin_error_message',$this->obj->lang->line('session_expire_error'));
			redirect('user/login','refresh');
		}	
	}
	...
	function get_language(){
		...
		$language_id = $this->obj->language_model->get_language_id($lang_code);
		if($lang_code==NULL) $lang_code = $this->obj->language_model->get_language_code($language_id);
		...
		$this->obj->template->assign('lang_code',$lang_code);
		return $language_id;
	}
