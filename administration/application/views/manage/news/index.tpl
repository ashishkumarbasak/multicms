							...
							<tbody>
								...
								<tr>
									...
									<td>
                                        ...
                                        <a href="{$baseurl}manage/news/edit/{$page->page_id}">{$page->page_title}</a>
                                    </td>
                                    <td>{$page->status}</td>
                                    <td>{$page->date_created}</td>
                                    <td>{$page->author}</td>
									<td>
                                    	<a class="btn btn-small" href="{$baseurl}manage/news/edit/{$page->page_id}">edit</a>
                                        <a class="btn btn-danger btn-small" href="{$baseurl}manage/news/delete/{$page->news_id}/{$page->page_id}" onclick='return confirm("Are you sure you want to delete?")'>Delete</a>
									</td>
								</tr>
                                    {foreach from=$subpages item=subpage name=listofsubpages key=spk}
                                    	<tr {if $subpage->mother_page_id eq "0"} {else} class="blue_background" {/if}>
                                            ...
                                            <td>
                                                <a class="btn btn-small" href="{$baseurl}manage/news/edit/{$subpage->page_id}">edit</a>
                                                <a class="btn btn-danger btn-small" href="{$baseurl}manage/products/delete/{$subpage->news_id}/{$subpage->page_id}" onclick='return confirm("Are you sure you want to delete?")'>Delete</a>
                                            </td>
                                        </tr>
                                    {/foreach}
								{/foreach}
