...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
		...
		if(isset($_SESSION['page_id']))
		{
			...
			if($page_id==NULL) $page_id= "temp";
		}
		else $page_id= "temp";
		//echo $page_id;
		$dir = dirname($this->get_server_var('SCRIPT_FILENAME')).'/'.$page_id;
		$thumd_dir = dirname($this->get_server_var('SCRIPT_FILENAME')).'/'.$page_id."/thumbnail";
		if(!is_dir($dir)) mkdir($dir,0777);
		if(!is_dir($thumd_dir)) mkdir($thumd_dir,0777);		
		
		$this->options = array(
            'script_url' => $this->get_full_url().'/',
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/'.$page_id.'/',
            'upload_url' => $this->get_full_url().'/'.$page_id.'/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    protected function get_file_object($file_name) {
        if ($this->is_valid_file_object($file_name)) {
			...
			$query = "select * from trv_page_videos where image_name like '%".$file_name."%' and page_id='".$_SESSION['page_id']."'";
